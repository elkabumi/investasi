...
switch ($page) {
	case 'list':
		...
		if(isset($_GET['preview'])){
			...
			log_data(10, 0, $_SESSION['user_id'], "dashboard realisasi investasi pma,pmdn & non fas");
		}else{
			$year_default = date('Y');
			$country_id = 0;
			$city_id = 0;
	...
	case 'form_result':
		...
		header("Location: dashboard_realisasi_investasi_triwulan.php?page=list&preview=1&year=$year&country_id=$country_id&city_id=$city_id&business_type_id=$business_type_id&sub_business_type=$sub_business_type");
	break;
}
